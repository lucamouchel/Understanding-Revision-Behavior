...
def get_last_index_where_written(user_index):
    """
    ...
    """
    return indices_of_first_attempts_per_user[user_index + 1] if  user_index != len(sorted_users) - 1 else len(df) - 1
...
def get_map_user_to_recipes():
    ...
    """
    return separate_sessions().groupby('user id')["recipe index in data"].apply(list)
